...
python ../clean_codes_v2/run_train_pipeline.py \
    ...
    --train 0 --N 9 --fresh_run 0 \
    > "${base_dir}/N9_${JOBID}.log" 2>&1 && \

python ../clean_codes_v2/run_train_pipeline.py \
    --config-file train_${ratio1}_${ratio2}.json \
    --train 0 --N 10 --fresh_run 0 \
    > "${base_dir}/N10_${JOBID}.log" 2>&1 && \

python ../clean_codes_v2/run_train_pipeline.py \
    --config-file train_${ratio1}_${ratio2}.json \
    --train 0 --N 11 --fresh_run 0 \
    > "${base_dir}/N11_${JOBID}.log" 2>&1 && \

python ../clean_codes_v2/run_train_pipeline.py \
    --config-file train_${ratio1}_${ratio2}.json \
    --train 0 --N 12 --fresh_run 0 \
    > "${base_dir}/N12_${JOBID}.log" 2>&1
